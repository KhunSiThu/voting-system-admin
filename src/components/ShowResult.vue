<template>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 justify-between w-full">
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow hover:shadow-lg transition">
            <div v-if="king" id="overallKing"
                ...
                <p class="w-full text-left font-bold text-gray-700 dark:text-gray-300">Total Votes - {{ king.voterCount
                    }}</p>
                ...
                <button @click="openModal(king, table + 'King')"
                    ...
                    <span v-if="allResults?.[table + 'King']">Published</span>
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow hover:shadow-lg transition">
            <div v-if="queen" id="overallQueen"
                ...
                <p class="w-full text-left font-bold text-gray-700 dark:text-gray-300">Total Votes - {{ queen.voterCount
                    }}</p>
                ...
                <button @click="openModal(queen, table + 'Queen')"
                    ...
                    <span v-if="allResults?.[table + 'Queen']">Published</span>
                    <span v-else>Publish</span>
                </button>
            </div>
            ...
            <div class="overflow-x-auto">
                <table
                    ...
                    <tbody v-if="resultsQueen">
                        <tr v-for="queen in resultsQueen" :key="queen.id"
        ...
        <div
            ...
            <div class="flex justify-center gap-4">
                <button @click="confirmPublish(publishData)"
                    ...
                    <span v-if="!click">Yes, Publish Result</span>
            ...
            <div class="mt-4 text-center">
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    If you're sure about the result, you can publish it. Otherwise, you can cancel and make further
                    adjustments.
                </p>
            </div>
        </div>
    ...
    <div v-if="showSuccessModal"
        ...
        <div
            class="bg-white rounded-lg p-8 shadow-xl dark:bg-gray-800 dark:text-white w-96 max-w-md transform transition-all duration-300 scale-95 hover:scale-100">
            <p class="text-base text-gray-700 dark:text-gray-300 text-center mb-4">
                Success! The result has been published successfully.
            </p>
            <div class="flex justify-center gap-4">
...
export default {
    ...
    setup(props) {
        ...
        const openModal = (candidate, objName) => {
            // Filter results by the provided objName
            const filterResults = props.allResults?.[objName] || null;

            // Check if filtered results are available
            if (filterResults) {
                // Show the success modal because the result is already published
                publishData.value = {
                    objName,
                    ...candidate
                };
                showModal.value = false; // Close the main modal if results are already there
                showSuccessModal.value = true; // Show the success modal
                return;
            }

            // Prepare the publish data for normal modal
            publishData.value = {
                objName,
                ...candidate
            };
